...
const AdminEditExhibit = () => {
  ...
  const { editExhibition, tinyLoader, allExhibitions, fetchExhibition } =
    useAdminContext();
  ...
  const currentProject = allExhibitions?.filter((x) => x?.id == id)[0];
      ...
      <div className="w-full md:pl-[300px] md:pr-8 px-3 pt-5 pb-[100px]">
        <div className="flex flex-col gap-8">
          <h1 className="text-[2rem]">
            Edit Exhibit - {currentProject?.title}{" "}
          </h1>
          ...
          <form className="w-full border-2 border-[#f97316] rounded-lg p-4 bg-neutral-200/50 flex flex-col gap-4">
            ...
            <button
              onClick={(e) => {
                e.preventDefault();
                editExhibition(formData, userData?.access, currentProject?.id);
              }}
